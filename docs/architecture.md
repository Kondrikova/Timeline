...
### 7.7 Инварианты, реализуемые на уровне хранилища

Перечисленные ниже правила проверяются в БД или в транзакции сервиса, а не только в
коде приложения: проверка «прочитали, убедились, записали» на уровне сервиса
обходится гонкой двух параллельных запросов.

| Инвариант | Сервис | Механизм |
|---|---|---|
| Спринты не пересекаются по датам | `schedule` | `EXCLUDE USING gist` |
| Отпуска одного сотрудника не пересекаются | `team` | `EXCLUDE USING gist` |
| Граф направленных связей ацикличен | `backlog` | Рекурсивный CTE в транзакции добавления связи |
| Внутри группы `SIMULTANEOUS` нет направленной связи | `backlog` | Свёртка групп (union-find) при добавлении связи |
| Одна оценка на пару задача × дисциплина | `backlog` | Составной первичный ключ |
| Одна аллокация на тройку задача × спринт × дисциплина | `planning` | Уникальный индекс |
| Событие обрабатывается потребителем однократно | все | Первичный ключ `(event_id, consumer)` |

Ограничения исключения требуют расширения `btree_gist` и создаются в миграциях
Flyway вместе с таблицами:

```sql
CREATE EXTENSION IF NOT EXISTS btree_gist;

-- schedule-service
ALTER TABLE sprint ADD CONSTRAINT sprint_no_overlap
  EXCLUDE USING gist (daterange(start_date, end_date, '[]') WITH &&);

-- team-service
ALTER TABLE vacation ADD CONSTRAINT vacation_no_overlap
  EXCLUDE USING gist (
    member_id WITH =,
...
-- planning-service
CREATE UNIQUE INDEX allocation_unique
  ON allocation (plan_version_id, task_id, sprint_id, discipline_id);
```

Проверка ацикличности выполняется перед вставкой направленной связи в той же
транзакции: если новый конец уже достижим из нового начала, связь создаст цикл и
отклоняется.

```sql
WITH RECURSIVE reachable(id) AS (
    SELECT to_task_id FROM task_link
     WHERE from_task_id = :newTo AND type IN ('SEQUENTIAL', 'BLOCKING')
    UNION
    SELECT l.to_task_id FROM task_link l
      JOIN reachable r ON l.from_task_id = r.id
     WHERE l.type IN ('SEQUENTIAL', 'BLOCKING')
)
SELECT EXISTS (SELECT 1 FROM reachable WHERE id = :newFrom);
-- true => связь создаст цикл, отклоняем
```

Задачи и связи обязаны находиться в одном сервисе и одной БД именно из-за этих двух
инвариантов: они охватывают всю связную компоненту графа, поэтому распределённая
проверка была бы подвержена гонкам.
...
| 2 | `team-service`: сотрудники, дисциплины с velocity, отпуска с ограничением непересечения в СУБД, outbox, первые события | **Сценарий 3 закрыт** |
| 3 | `schedule-service`: спринты с ограничением непересечения в СУБД, производственный календарь | Спринты ведутся и валидируются |
| 4 | `backlog-service`: эпики, задачи, роли и оценки в SP, связи трёх типов с проверкой ацикличности и противоречий (раздел 7.7) | **Сценарий 1 закрыт** |
...
| 026 | Инварианты непересечения и ацикличности — на уровне хранилища | Проверка только в коде сервиса | Проверка «прочитали, убедились, записали» обходится гонкой двух параллельных запросов; список механизмов в разделе 7.7 |
