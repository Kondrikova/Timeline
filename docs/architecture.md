...
|---|---|
| Команд | 1 |
| Пользователей | десятки |
| Задач | сотни |
| Спринтов на горизонте | ~26 |
| Профессиональных ролей | ~10 |
| Объём данных | единицы мегабайт |
| Разработчиков | 1 |

### Область применимости архитектуры

Данные и нагрузка малы: технически задача решается модульным монолитом, и для
продуктовой разработки это была бы предпочтительная опция. Микросервисная
декомпозиция выбрана как **целевое требование проекта** (учебная работа по
микросервисной архитектуре). Поэтому в документе для каждого паттерна явно
зафиксировано, решает ли он реальную проблему системы или демонстрируется;
отказы от паттернов обоснованы наравне с их применением.
...
| Термин | Значение |
|---|---|
| **TeamMember** | Сотрудник команды: дисциплина, FTE, период активности, отпуска |
| **FTE** | Доля ставки сотрудника в команде, 0.0–1.0 |
...
| **TaskEstimate** | Оценка задачи для одной дисциплины, в человеко-днях |
| **Dependency** | Зависимость между задачами: тип + лаг в спринтах |
| **Allocation** | Плановый объём работ: задача × спринт × дисциплина × человеко-дни |
| **Capacity** | Доступная ёмкость дисциплины в спринте, человеко-дни. Вычисляется |
...
**Оценка задачи — вектор по дисциплинам, а не скаляр.** Задача требует работы
нескольких ролей в разном объёме и в разное время. Без разбиения оценки по
дисциплинам невозможно вычислить пересечение загрузки с ёмкостью роли, то есть
теряется основная ценность продукта.

**Единица оценки — человеко-дни, не story points.** Ёмкость считается в
человеко-днях. Конвертация story points в дни через velocity на уровне отдельной
дисциплины статистически несостоятельна.

**Единица времени планирования — спринт, а не день.** Зависимость `FINISH_TO_START`
сводится к `successor.firstSprint > predecessor.lastSprint + lag`. Это позволяет
обойтись топологической сортировкой вместо полноценного CPM-планировщика и
кратно снижает сложность ядра.
...
Разбиение по транзакциям сознательно не применялось как технический подход,
приводящий к анемичным CRUD-сервисам.
...
Сервисы неравноценны. Вся существенная сложность — каскадный пересчёт зависимостей,
расчёт ёмкости, детекция конфликтов — сосредоточена в `planning-service`. Остальные
доменные сервисы близки к CRUD. Это осознанное распределение: ядро изолировано и
может развиваться независимо от справочных данных вокруг него.

### Почему нет сервиса авторизации

Аутентификация — инфраструктурная функция, она вынесена во внешний провайдер
(Keycloak). Авторизация — сквозная функция, она не централизуется, потому что
значительная часть правил предметна. Правило «сотрудник редактирует только свой
отпуск» — инвариант агрегата `TeamMember`; централизованный сервис авторизации не
смог бы его проверить, не зная модель команды. Выделение «сервиса авторизации» было
бы разбиением по техническому слою, а не по бизнес-возможности.

### Почему календарь и планирование разделены

Календарь итераций — стабильный справочник с низким темпом изменений и почти без
логики. Планирование — ядро с высокой сложностью и высоким темпом изменений.
Объединение дало бы модуль с разной ценой ошибки в разных частях.

### Почему планирование не разделено дальше

Каскадный сдвиг зависимостей за одну операцию изменяет сотни аллокаций. Пока они в
одной БД — это обычная ACID-транзакция. Разделение «спринты» / «аллокации» /
«конфликты» превратило бы внутренний пересчёт в распределённую транзакцию с
компенсациями, то есть создало бы саги на ровном месте. Это граница, которую резать
нельзя.

### Почему расчёт ёмкости внутри планирования

Ёмкость вычисляется из состава команды, отпусков (владелец `team-service`), дат
спринтов и производственного календаря (владелец `schedule-service`). Единственный
потребитель — планирование. Вынесение расчёта в отдельный сервис добавило бы сетевой
вызов в самый горячий путь системы, не дав ни одного независимого потребителя.
Планирование держит локальные реплики исходных данных и считает ёмкость локально.

Альтернатива, отклонённая: расчёт в `team-service` — потребовал бы реплицировать
туда календарь и спринты, то есть развернуть ту же связность в обратную сторону.
...
Инвариант непересечения отпусков одного сотрудника обеспечивается СУБД:
...
Валидация непересечения спринтов — на уровне СУБД, а не приложения:
...
Это прямое выполнение требования «спринты: валидация» на самом надёжном уровне —
...
**Инвариант ацикличности графа зависимостей шире одного агрегата** `Task`: он
затрагивает два агрегата и всю связную компоненту графа. Поэтому он защищается на
уровне транзакции сервиса, а не агрегата. Практическое следствие: задачи и
зависимости обязаны находиться в одном сервисе и одной БД, иначе проверка становится
распределённой и подверженной гонкам.

Проверка при добавлении зависимости:

```sql
WITH RECURSIVE reachable(id) AS (
    SELECT successor_id FROM task_dependency WHERE predecessor_id = :newSuccessor
    UNION
    SELECT d.successor_id FROM task_dependency d JOIN reachable r ON d.predecessor_id = r.id
)
SELECT EXISTS (SELECT 1 FROM reachable WHERE id = :newPredecessor);
-- true => добавление создаст цикл, отклоняем
```
...
- **Производственный календарь.** «Рабочие дни спринта» нельзя считать как
  ...
  половины итерации.
- **FTE.** Частичная занятость и шаринг сотрудника между командами.
